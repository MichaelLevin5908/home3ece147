...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # First layer: X (N, D) dot W1.T (D, H) + b1 (H,) => (N, H)
    z1 = X.dot(W1.T) + b1
    # ReLU activation
    a1 = np.maximum(0, z1)
    # Second layer: a1 (N, H) dot W2.T (H, C) + b2 (C,) => (N, C)
    scores = a1.dot(W2.T) + b2
    ...
    # Gradient of scores (softmax gradient)
    ...
    grads['W2'] = dscores.T.dot(a1) + reg * W2  # (C, H)
    grads['b2'] = np.sum(dscores, axis=0)  # (C,)

    # Backprop into hidden layer
    # da1 = dscores dot W2 => (N, H)
    da1 = dscores.dot(W2)  # (N, H)
    ...
    # z1 = X.dot(W1.T) + b1
    grads['W1'] = dz1.T.dot(X) + reg * W1  # (H, D)
