...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    # Backprop through layer 1 (affine + relu)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    # Backprop through last layer (affine only)
    dout, grads['W{}'.format(self.num_layers)], grads['b{}'.format(self.num_layers)] = affine_backward(dout, caches[self.num_layers])
    grads['W{}'.format(self.num_layers)] += self.reg * self.params['W{}'.format(self.num_layers)]

    # Backprop through hidden layers (affine-relu)
    for i in range(self.num_layers - 1, 0, -1):
      dout, grads['W{}'.format(i)], grads['b{}'.format(i)] = affine_relu_backward(dout, caches[i])
